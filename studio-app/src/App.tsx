...
export default function App() {
  ...
  const currentPost =
    posts.find((p) => p.post_id === currentPostId) ||
    posts.find((p) => p.status === 'recording') ||
    posts[0]
  const currentIndex = currentPost ? posts.findIndex((p) => p.post_id === currentPost.post_id) : -1
  const upcomingPosts =
    currentIndex >= 0 ? posts.slice(currentIndex, currentIndex + 6) : posts.slice(0, 6)
    ...
    <div className="studio-container">
      ...
      <main className="main-content">
        ...
        <div className="current-post">
          {currentPost ? (
            <>
              <div className="current-label">Active post</div>

              <div className="current-type">
                <span className="type-icon">{currentPost.type_icon || '📄'}</span>
                <span className="type-name">
                  {currentPost.type_display || currentPost.type_key}
                </span>
              </div>

              <div className="current-title">{currentPost.title || '(No title)'}</div>
              ...
              <div className="timer-container">
                <div
                  className={`timer ${isRecording ? 'recording' : ''} ${elapsedTime > currentPost.duration_sec ? 'overtime' : ''}`}
                >
                  {formatTime(elapsedTime)}
                </div>
                <div className="timer-target">/ {formatTime(currentPost.duration_sec)}</div>
              </div>
            </>
          ) : (
            <div className="no-post">
              <div className="no-post-icon">📋</div>
              <div className="no-post-text">No posts in this programme</div>
            </div>
          )}
        </div>
        ...
        <div className="post-queue">
          <div className="queue-header">Upcoming posts</div>
            ...
            {upcomingPosts.map((post, idx) => (
              ...
              >
                ...
                <div className="queue-info">
                  ...
                  <div className="queue-title">{post.title || '(No title)'}</div>
                </div>
      ...
      <div className="control-bar">
        ...
        <button
          className="control-btn btn-approve"
          onClick={handleApprove}
          disabled={!currentPost || currentPost.status !== 'inspelad'}
        >
          ✓ Approve
        </button>
