/**
 * THAST CHURCH SERVICE PRODUCTION SYSTEM
...
const SYSTEM_NAME = 'Church Service Production System'
...
const POST_SCHEMA = {
  ...
  TYPE: 3, // Post type (sermon, hymn, reading, etc)
  TITLE: 4, // Primary content/title
  DURATION: 5, // Duration in seconds (integer)
  PEOPLE_IDS: 6, // Comma-separated person IDs
  LOCATION: 7, // pulpit, altar, pew, etc
  INFO_POS: 8, // Technical info/position
  GRAPHICS: 9, // Graphics cues
  NOTES: 10, // Additional notes
  RECORDING_DAY: 11, // Day 1, Day 2, Day 3
  RECORDING_TIME: 12, // Estimated clock time (calculated)
  STATUS: 13, // planned, recording, recorded, approved
  TEXT_AUTHOR: 14, // Text author (optional, for music)
  COMPOSER: 15, // Composer (optional, for music)
  ARRANGER: 16, // Arranger (optional)
...
const PERSON_SCHEMA = {
  ...
  NAME: 1, // Full name
  ROLES: 2, // Comma-separated roles (liturgist, preacher, etc)
  CONTACT: 3, // Email or phone
  TYPE: 4, // participant, team, composer, text_author
  CREATED: 5,
}

const PERSON_HEADERS = ['person_id', 'name', 'roles', 'contact', 'type', 'created']
...
const PROGRAM_SCHEMA = {
  ...
  CHURCH_YEAR: 4, // Church calendar (e.g. "2nd Sunday of Lent")
...
const POST_TYPE_SCHEMA = {
  ...
  REQUIRES_COMPOSER: 6,
  CATEGORY: 7, // liturgical, music, presentation, etc
...
const VIEW_CONFIG = {
  ...
  // Programme view columns (simplified, single view)
  PROGRAM_VIEW: {
    COLUMNS: [
      'NR',
      'Type',
  ...
  // Schedule view columns (aggregated across all programmes)
  SCHEDULE_VIEW: {
    COLUMNS: [
      'Day',
      'Time',
...
const RECORDING_DAYS = {
  DAY_1: { key: 'day1', display: 'Day 1', description: 'Scripture Reading & Sermon' },
  DAY_2: { key: 'day2', display: 'Day 2', description: 'Music & Choir' },
  DAY_3: { key: 'day3', display: 'Day 3', description: 'Congregation (full service)' },
}
...
const POST_STATUS = {
  PLANNED: { key: 'planned', display: 'Planned', colour: '#FFFFFF' },
  RECORDING: { key: 'recording', display: 'Recording', colour: '#FFD54F' },
  RECORDED: { key: 'recorded', display: 'Recorded', colour: '#A5D6A7' },
  APPROVED: { key: 'approved', display: 'Approved', colour: '#81C784' },
}
