...
function createProgramView_(programNr) {
  ...
  sheet.getRange('A1').setValue(`PROGRAMME ${programNr} - RUN SHEET`)
  ...
  sheet
    .getRange('B2')
    .setFormula(`=IFERROR(INDEX(_DB_Program!B:B,MATCH(${programNr},_DB_Program!A:A,0)),"")`)
  sheet.getRange('D2').setValue('Recording start:')
  sheet
    .getRange('E2')
    .setFormula(`=IFERROR(INDEX(_DB_Program!C:C,MATCH(${programNr},_DB_Program!A:A,0)),"")`)
  sheet.getRange('G2').setValue('Broadcast:')
  sheet
    .getRange('H2')
    .setFormula(`=IFERROR(INDEX(_DB_Program!D:D,MATCH(${programNr},_DB_Program!A:A,0)),"")`)

  // Programme metadata row 3
  sheet.getRange('A3').setValue('Church year:')
  sheet
    .getRange('B3')
    .setFormula(`=IFERROR(INDEX(_DB_Program!E:E,MATCH(${programNr},_DB_Program!A:A,0)),"")`)
  sheet.getRange('D3').setValue('Prod no:')
  sheet
    .getRange('E3')
    .setFormula(`=IFERROR(INDEX(_DB_Program!F:F,MATCH(${programNr},_DB_Program!A:A,0)),"")`)
  sheet.getRange('G3').setValue('Target:')
  sheet
    .getRange('H3')
    .setFormula(
      `=TEXT(IFERROR(INDEX(_DB_Program!G:G,MATCH(${programNr},_DB_Program!A:A,0))/86400,"00:00:00"),"[hh]:mm:ss")`,
    )
  ...
  sheet
    .getRange('B4')
    .setFormula(`=TEXT(SUMIF(_DB_Posts!B:B,${programNr},_DB_Posts!F:F)/86400,"[hh]:mm:ss")`)
  sheet.getRange('D4').setValue('Post count:')
  sheet.getRange('E4').setFormula(`=COUNTIF(_DB_Posts!B:B,${programNr})`)
  sheet.getRange('G4').setValue('Recording days:')
  sheet
    .getRange('H4')
    .setFormula(
      `=JOIN(", ",UNIQUE(FILTER(_DB_Posts!L:L,_DB_Posts!B:B=${programNr},_DB_Posts!L:L<>"")))`,
    )
  ...
  sheet
    .getRange('A1')
    .setNote(
      'This is a dynamic view based on the database _DB_Posts.\\n\\n' +
        'To add posts: Use the menu "Posts > Add new post"\\n' +
        'To edit: Change directly in the cells below\\n' +
...
function createScheduleView_() {
  ...
  sheet.getRange('A1').setValue('RECORDING SCHEDULE - ALL PROGRAMMES')
  sheet.getRange('A1').setFontSize(16).setFontWeight('bold').setHorizontalAlignment('center')
  sheet.getRange('A1').setBackground(COLOURS.HEADER_BG).setFontColor(COLOURS.HEADER_TEXT)
  sheet.setRowHeight(1, 40)

  // Statistics row
  sheet.getRange('A2').setValue('Total recording time:')
  sheet.getRange('B2').setFormula(`=TEXT(SUM(_DB_Posts!F:F)/86400,"[hh]:mm:ss")`)
  sheet.getRange('D2').setValue('Total post count:')
...
function createOverviewView_() {
  ...
  sheet.getRange('A1').setValue('OVERVIEW - PRODUCTION DASHBOARD')
  ...
  sheet.getRange('A3').setValue('PROGRAMME OVERVIEW')
  ...
  sheet
    .getRange('A4:G4')
    .setBackground(COLOURS.HEADER_BG)
    .setFontColor(COLOURS.HEADER_TEXT)
    .setFontWeight('bold')
  ...
  for (let i = 1; i <= 4; i++) {
    ...
    sheet
      .getRange(`B${row}`)
      .setFormula(`=IFERROR(INDEX(_DB_Program!B:B,MATCH(${i},_DB_Program!A:A,0)),"")`)
    sheet.getRange(`C${row}`).setFormula(`=COUNTIF(_DB_Posts!B:B,${i})`)
    sheet
      .getRange(`D${row}`)
      .setFormula(`=TEXT(SUMIF(_DB_Posts!B:B,${i},_DB_Posts!F:F)/86400,"[hh]:mm:ss")`)
    sheet
      .getRange(`E${row}`)
      .setFormula(`=COUNTIFS(_DB_Posts!B:B,${i},_DB_Posts!N:N,"${POST_STATUS.RECORDED.key}")`)
    sheet
      .getRange(`F${row}`)
      .setFormula(`=COUNTIFS(_DB_Posts!B:B,${i},_DB_Posts!N:N,"${POST_STATUS.APPROVED.key}")`)
    sheet
      .getRange(`G${row}`)
      .setFormula(`=IF(C${row}>0,ROUND((E${row}+F${row})/C${row}*100,0)&"%","0%")`)
  }
  ...
  sheet.getRange('A11').setValue('RECORDING BREAKDOWN')
  sheet.getRange('A11').setFontSize(14).setFontWeight('bold')
  sheet.setRowHeight(11, 35)

  const dayHeaders = ['Day', 'Post count', 'Total time', 'Description']
  ...
  Object.values(RECORDING_DAYS).forEach((day, index) => {
    ...
    sheet
      .getRange(`C${row}`)
      .setFormula(`=TEXT(SUMIF(_DB_Posts!L:L,"${day.key}",_DB_Posts!F:F)/86400,"[hh]:mm:ss")`)
...
function createCreditsView_() {
  ...
  sheet.getRange('A1').setValue('CREDITS - PARTICIPANTS AND STAFF')
  sheet.getRange('A1').setFontSize(16).setFontWeight('bold').setHorizontalAlignment('center')
  sheet.getRange('A1').setBackground(COLOURS.HEADER_BG).setFontColor(COLOURS.HEADER_TEXT)
  sheet.setRowHeight(1, 40)

  sheet.getRange('A2').setValue('This list is automatically generated from _DB_People')
  ...
  // Headers
  const headers = ['ID', 'Name', 'Role', 'Contact', 'Type', 'Added']
  const headerRange = sheet.getRange(4, 1, 1, headers.length)
  headerRange.setValues([headers])
  headerRange.setBackground(COLOURS.HEADER_BG)
  headerRange.setFontColor(COLOURS.HEADER_TEXT)
